...
for k=1:length(SNRdB)
    ...
    [r, pulse, Es] = PulseShape( s, 'SINC', Ns, N, 0.25);
    ...
    if k == length(SNRdB)
        display(' ')
        display(['BER with no processing = ',num2str(ber_no_processing(k))])
        display(['BER using LMS = ',num2str(ber_lms(k))])
        display(['BER using RLS = ',num2str(ber_rls(k))])
        display(['BER using DD LMS = ',num2str(ber_lms_dd(k))])
        display(['BER using DD RLS = ',num2str(ber_rls_dd(k))])
        display(' ')
      end
end
