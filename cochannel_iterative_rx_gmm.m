function [ b1_est, b2_est, theta, sig_stop, sig_nan] = cochannel_iterative_rx_gmm( r, sys, user, Xconst_fctr, h1_est, h2_est, gmm_est, fg1_ind, fg2_ind, x1_fg_ind, x2_fg_ind, b1_fg_ind, b2_fg_ind , b1 , b2 , x1 , x2 , h1 , h2 , gmm)
%COCHANNEL_ITERATIVE_RX_GMM Factor graph and message passing for cochannel
...
%       inherited from "cochannel_iterative_rx_gmm.m"
%       after major changes to that script
...
for w=1:sys.numIter
    ...
    for z=1:sys.numDetectIter
        ...
        for i=1:sys.dframeLen
            ...
            Xout = factor_detect_approx_gmm( y(i) , Xin , hIN , theta(w).gmm , sys.C , Xconst_fctr , sys.eta );
    ...
    [ theta(w+1).gmm(1).lam, theta(w+1).gmm(2).lam, theta(w+1).gmm(1).sig, theta(w+1).gmm(2).sig, theta(w+1).h1, theta(w+1).h2 ] = cochannel_est_gmm( r, sys, Xconst_fctr, theta(w).h1, theta(w).h2, theta(w).gmm, SX_u1, SX_u2 );
...
for ww=w+1:sys.numIter
    ...
    for z=1:sys.numDetectIter
        ...
        for i=1:sys.dframeLen
            ...
            Xout = factor_detect_approx_gmm( y(i) , Xin , hIN , theta(ww).gmm , sys.C , Xconst_fctr , sys.eta );
            X_u1(i,colIdx) = Xout.';
    ...
    % ECM noise and channel parameter estimation
    SX_u1 = [[S_u1; tmp1] [tmp2; X_u1]];
    [ theta(ww+1).gmm(1).lam, theta(ww+1).gmm(2).lam, theta(ww+1).gmm(1).sig, theta(ww+1).gmm(2).sig, theta(ww+1).h1 ] = cochannel_est_gmm_U1( rr, sys, Xconst_fctr, theta(ww).h1, theta(ww).gmm, SX_u1 );
...
for ww=w+1:sys.numIter
    ...
    % ECM noise and channel parameter estimation
    SX_u2 = [[S_u2; tmp1] [tmp2; X_u2]];
    [ theta(ww+1).gmm(1).lam, theta(ww+1).gmm(2).lam, theta(ww+1).gmm(1).sig, theta(ww+1).gmm(2).sig, theta(ww+1).h2 ] = cochannel_est_gmm_U1( rr, sys, Xconst_fctr, theta(ww).h2, theta(ww).gmm, SX_u2 );
