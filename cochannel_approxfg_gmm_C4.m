function [ b1_est, b2_est, sig_stop, sig_nan] = cochannel_approxfg_gmm_C4( r, sys, user, Xconst_fctr, h1, h2, gmm, fg1_ind, fg2_ind, x1_fg_ind, x2_fg_ind, b1_fg_ind, b2_fg_ind )
%COCHANNEL_APPROXFG_GMM_C4 Factor graph and message passing for cochannel
...
%
%       inherited from "cochannel_approxfg_gmm.m"
...
for w=1:sys.numIter
    ...
    for z=1:sys.numDetectIter
        ...
        for i=1:sys.frameLen
            ...
            hIN = [ h1( hIdx ); h2( hIdx ) ];
            Xout = factor_detect_approx_gmm_C4( r(i) , Xin , hIN , gmm , Xconst_fctr , sys.eta );
...
for ww=w+1:sys.numIter
    ...
    for z=1:sys.numDetectIter
        ...
        for i=1:sys.frameLen
            ...
            Xin = X_u1(i,colIdx).';
            hIN = h1( hIdx );
            Xout = factor_detect_approx_gmm_C4( rr(i) , Xin , hIN , gmm , Xconst_fctr , sys.eta );
            X_u1(i,colIdx) = Xout.';
